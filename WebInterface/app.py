...
def profile_tasks():
    ...
    results = tasks[["task_id", "thumbnail"]]

    results = results.rename(columns={"task_id": "name", "thumbnail": "url"})
...
def upload_text():
    ...
    if request.method == "POST":
        ...
        # thumbnail = WebInterface/static/text_thumbnail.png
        # save the thumbnail as base64 in the database
        from PIL import Image

        # thumbnail = Image.open("static/text_thumbnail.png")
        # thumbnail = thumbnail.resize((320, 180))
        # thumbnail = base64.b64encode(thumbnail.tobytes()).decode("utf-8")
        ...
        add_task_with_thumbnail(task_id, thumbnail)
...
def check_text_status(task_id):
    global client
    if check_if_user_has_task(current_user.id, task_id):
        return get_task_by_id(task_id).iloc[0]["result"], 286
